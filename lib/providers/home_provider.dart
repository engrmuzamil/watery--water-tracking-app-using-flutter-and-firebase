...
class HomeProvider extends ChangeNotifier {
  ...
  late WeeklyData _weeklyData;
  late String _uid;
  late AppUser _appUser;

  late CollectionReference _weekColRef;
  late DocumentReference _userRef;
  late DocumentReference _currentWeek;
  ...
  Future<void> init() async {
    if (_isInited == false) {
      try {
        ...
        if (!snapshot.exists) {
          Map<String, dynamic> newWeek = WeeklyData().createNewWeek(
              docId, _today.year, _today.month, week, _appUser.dailyTarget);
        ...
        if (canGetLocation) {
          ...
          final Uri link = Uri.https(
            ...
            'data/2.5/weather?lat=${_locationData.latitude}&lon=${_locationData.longitude}&appid=$keys&units=metric',
          );
